 ...
 * http://developers.gigya.com/display/GD/socialize.showLoginUI+JS
 * @endlink
 */
 ...
 * configuration parameters to the Javascript settings. A list of available
 * parameters can be found in the Gigya developers site: @link
 * http://developers.gigya.com/display/GD/socialize.showAddConnectionsUI+JS
 * @endlink
 */
 ...
 * @endlink A list of available parameters can be found at the Gigya developers
 * site: @link
 * http://developers.gigya.com/display/GD/socialize.showReactionsBarUI+JS
 * @endlink
 */
 ...
 * http://developers.gigya.com/display/GD/socialize.showShareUI+JS
 ...
 * http://developers.gigya.com/display/GD/accounts.socialLogin+REST#accounts.socialLoginREST-ResponseData
...
function hook_gigya_raas_create_user_alter(&$new_user_array, $gigya_info) {
  ...
  // Adding "the grate" suffix to the first name field
  $new_user_array['filed_firstName']['und'][0]['value'] = $new_user_array['filed_firstName']['und'][0]['value'] . " the grate";
}
...
function hook_gigya_raas_update_user_alter(&$edit, $user, $gigya_info) {
  // Use the user first name as a drupal user name
  $edit['name'] = $gigya_info['firstName'];
  // Adding "the grate" suffix to the first name field
  $edit['filed_firstName']['und'][0]['value'] = $edit['filed_firstName']['und'][0]['value'] . " the grate";
}
 ...
 * http://developers.gigya.com/display/GD/gm.showAchievementsUI+JS (Achievements plugin)
 * http://developers.gigya.com/display/GD/gm.showChallengeStatusUI+JS (Challenge plugin)
 * http://developers.gigya.com/display/GD/gm.showLeaderboardUI+JS (Leaderboard plugin)
 * http://developers.gigya.com/display/GD/gm.showUserStatusUI+JS (UserStatus plugin)
...
/**
 * Hook that lets other alter the ajax commands array that is returned after login/register via gigya.
 ...
 */
function hook_gigya_login_register_response_alter(&$commands, $ctx, $drupal_user, $gigya_user) {
...
/**
 * Hook that lets others alter the info from gigya edit profile before it is saved to drupal.
 *
 ...
 */
function hook_gigya_profile_update_alter(&$edit, $user, $gigya_account) {
  // Use the user first name as a drupal user name
